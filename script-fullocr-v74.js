document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('level-major').addEventListener('change', function(){
    const phase = document.getElementById('level-phase');
    phase.innerHTML = '<option value="">-- 請先選大等級 --</option>';
  ...
  document.getElementById('calc-manual').addEventListener('click', () => {
    ...
    let sec3 = 0;
    for(let i=pi; i<arr.length; i++){
      sec3 += (i===pi ? (arr[i]-cur) : arr[i]) / spd;
    }
    ...
    const reachable = realmDate <= epochEnd
      ? (majorOrder[majorOrder.indexOf(major)+1] + '後期')
      : (major + ['前期','中期','後期'][pi]);
    const lines = [
